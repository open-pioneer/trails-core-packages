...
describe("services", () => {
    ...
    it("should allow access to all services via react hook", async () => {
        function TestComponent() {
            const services = useServicesInternal<unknown>(
                "test",
                "test.Provider"
            ) as TestProvider[];
...
function createIntegration(options?: {
    ...
    intl?: MaybeReactive<PackageIntl>;
    ...
    // PackageRepr and ServiceRepr need an actual signal.
    const intlSignal = computed(() => getReactive(intl));

...
/**
 * Class added to the web component element.
 *
 * NOTE: Only used when shadow root is disabled, since we need something to match against.
 * (web component tags are not known to the application at this time).
 */
export const APP_CONTAINER_CLASS = "pioneer-container";
 ...
 */
export function getBuiltinStyles(rootNode: RootNode, host: HTMLElement) {
    ...
    const selector = isShadowRoot(rootNode) ? ":host" : host.tagName.toLowerCase();

...
import { createAppRoot, getBuiltinStyles, getStylesRoot, isShadowRoot, RootNode } from "../dom";
...
export class AppInstance {
    ...
    private stylesMount: Resource | undefined;
    ...
    private reset() {
        ...
        this.stylesMount = destroyResource(this.stylesMount);
    }
    ...
    private initStyles() {
        const builtinStyles = getBuiltinStyles(this.rootNode, this.options.hostElement);
        ...
        let stylesWatch: Resource | undefined;
        if (import.meta.hot) {
            appStyles?.on?.("changed", () => {
                LOG.debug("Application styles changed");
                applyStyles(appStylesNode, appStyles);
            });
        }

        let stylesNodes: HTMLElement[] | undefined = [builtinStylesNode, appStylesNode];
        getStylesRoot(this.rootNode).append(...stylesNodes);
        this.stylesMount = {
            destroy: () => {
                stylesNodes?.forEach((node) => node.remove());
                stylesNodes = undefined;
                stylesWatch = destroyResource(stylesWatch);
            }
        };
    }
